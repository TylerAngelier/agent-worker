...
## Overview
Agent Worker is a single-process CLI tool built in TypeScript, compiled to native binaries via `bun build --compile`. It follows a **poll-dispatch-update loop** pattern: poll Linear for ready tickets, dispatch work through a hook → code executor → hook pipeline, and update ticket status based on the outcome (per FR-01, FR-02, FR-10, FR-11). The code executor is pluggable — currently supporting Claude Code and OpenAI Codex via a `CodeExecutor` interface.
...
- **Responsibilities**:
  ...
  - Execute the Pipeline (pre-hooks → code executor → post-hooks)
...
### Pipeline
- **Purpose**: Execute the pre-hook → code executor → post-hook sequence for a single ticket.
- **Responsibilities**:
  - Run pre-hook commands sequentially; abort on first failure
  - Invoke the configured code executor with ticket title + description
  - Enforce executor timeout
...
### Code Executor (Interface + Implementations)
- **Purpose**: Pluggable interface for invoking AI coding tools as headless subprocesses.
- **Responsibilities**:
  - Define a common `CodeExecutor` interface (`name`, `run()`)
  - Provide implementations for Claude Code (`claude --print --dangerously-skip-permissions -p <prompt>`) and Codex (`codex exec --full-auto <prompt>`)
  - Run in the configured repo directory
  - Enforce timeout via process kill
  - Capture stdout/stderr for logging
  - Return success/failure + output
- **Interface**: `CodeExecutor.run(prompt: string, cwd: string, timeoutMs: number, logger: Logger): Promise<ExecutorResult>`
- **Factory**: `createExecutor(type: "claude" | "codex"): CodeExecutor`
...
- **Headless CLI over Agent SDK**: Coding tools are invoked as headless CLI subprocesses (e.g. `claude --print ...`, `codex exec ...`), not via SDKs. This keeps the worker language-agnostic and avoids coupling to any vendor's SDK. A `CodeExecutor` interface and factory make it trivial to add new tools. — Alternative: Agent SDK (richer control, but locks to a specific vendor).
...
| `claude` CLI | Headless AI code execution — Claude Code (runtime dependency, not bundled) | User-installed |
| `codex` CLI | Headless AI code execution — OpenAI Codex (runtime dependency, not bundled) | User-installed |
...
│   │   ├── executor.ts       # CodeExecutor interface, factory, shared helpers
│   │   ├── claude-executor.ts# Claude Code executor implementation
│   │   ├── codex-executor.ts # OpenAI Codex executor implementation
...
| Code executor non-zero exit | Abort pipeline, no post-hooks | → Failed + comment (retry if configured) |
| Code executor timeout | Kill process, abort pipeline | → Failed + comment (retry if configured) |
...
| Code executor started | info | ticket ID, executor type, timeout |
| Code executor output | debug | ticket ID, stdout (truncated) |
...
- **Executor prompt**: The ticket title and description are passed as a string argument to the code executor CLI. This is not shell-interpolated — it is passed programmatically via spawn args, avoiding injection.
...
## Assumptions
- [ASSUMED] `claude` CLI supports `--print` flag for headless invocation and returns exit code 0 on success.
- [ASSUMED] `codex` CLI supports `exec --full-auto` for headless invocation and returns exit code 0 on success.
