...
- Additional AI coding tools (Aider, etc.) behind the existing `CodeExecutor` interface.
...
## Assumptions
- [ASSUMED] The user has the configured agent harness (Claude Code or Codex) installed and available on PATH (the tool does not install or manage it).
