...
export function createClaudeExecutor(options?: ClaudeExecutorOptions): CodeExecutor {
  const execLog = log.child("claude");
  return {
    name: "claude",
    needsWorktree: true,
    async run(prompt: string, cwd: string, timeoutMs: number): Promise<ExecutorResult> {
      return time("claude", async () => {
      execLog.info("Claude Code started", { timeoutMs, model: options?.model });
      ...
      const [stdout, stderr] = await Promise.all([
        streamToLines(proc.stdout as ReadableStream<Uint8Array>, (line) => {
          execLog.info("claude", { stream: "stdout", line });
        }),
        streamToLines(proc.stderr as ReadableStream<Uint8Array>, (line) => {
          execLog.info("claude", { stream: "stderr", line });
        }),
      ]);
      ...
      if (timedOut) {
        execLog.error("Claude Code timed out", { timeoutMs });
        return { success: false, output, timedOut: true, exitCode: null };
      }

      if (exitCode !== 0) {
        execLog.error("Claude Code failed", { exitCode });
      } else {
        execLog.info("Claude Code completed successfully");
      }
