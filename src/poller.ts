...
export function createPoller(options: {
  ...
  const log = rootLog.child("poller");
  let isRunning = false;
  ...
  return {
    async start() {
      ...
      while (isRunning) {
        ...
        try {
          const tickets = await time("poll.fetchReadyTickets", () => options.provider.fetchReadyTickets());
          if (tickets.length > 0) {
            ...
            try {
              await time("poll.onTicket", () => options.onTicket(ticket));
            } catch (err) {
