...
/** Lazy child logger — resolves against the current singleton after init. */
function logger() { return log.child("poller"); }
  ...
  return {
    async start() {
      ...
      while (isRunning) {
        ...
        try {
          const tickets = await time(`poll #${pollCount} fetch`, () =>
          ...
          if (tickets.length > 0) {
            ...
            try {
              await time(`poll #${pollCount} handle ${ticket.identifier}`, () =>
