...
export async function processTicket(options: {
  ...
  // Lazy-init: created inside the function so the global `log` singleton
  // is already wired when called (test suites call initLogger first).
  const logger = log.child("scheduler");

  return time(`processTicket:${ticket.identifier}`, async () => {
