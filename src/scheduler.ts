...
import { log as logRoot, time } from "./logger.ts";

function getLog() {
  return logRoot.child("scheduler");
}
...
export async function processTicket(options: {
  ...
  try {
    ...
    getLog().info("Ticket claimed", { ticketId: ticket.identifier });
  } catch (err) {
    getLog().warn("Failed to claim ticket", {
  ...
  for (let attempt = 0; attempt <= config.executor.retries; attempt++) {
    ...
    try {
      lastResult = await time(`pipeline:${ticket.identifier}:attempt=${attempt}`, () =>
        executePipeline({
          ticket,
      ...
      getLog().error("Pipeline threw unexpected error", {
        ticketId: ticket.identifier,
        attempt,
        error: err instanceof Error ? err.message : String(err),
      });
  ...
  try {
    if (lastResult?.success) {
      ...
      getLog().info("Ticket in code review", { ticketId: ticket.identifier });
    ...
    getLog().error("Failed to update ticket status", {
      ticketId: ticket.identifier,
      error: err instanceof Error ? err.message : String(err),
    });
    return { outcome: "failed" };
