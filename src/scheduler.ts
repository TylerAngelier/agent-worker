...
export async function processTicket(options: {
  ...
  try {
    ...
    getLogger().info("Ticket claimed", { ticketId: ticket.identifier });
  } catch (err) {
    getLogger().warn("Failed to claim ticket", {
  ...
  for (let attempt = 0; attempt <= config.executor.retries; attempt++) {
    if (attempt > 0) {
      getLogger().warn("Retrying pipeline", {
        ticketId: ticket.identifier,
        attempt,
    ...
    try {
      ...
      getLogger().error("Pipeline threw unexpected error", {
        ticketId: ticket.identifier,
        attempt,
        error: err instanceof Error ? err.message : String(err),
      });
  ...
  try {
    if (lastResult?.success) {
      ...
      getLogger().info("Ticket in code review", { ticketId: ticket.identifier });
    ...
    getLogger().error("Failed to update ticket status", {
      ticketId: ticket.identifier,
      error: err instanceof Error ? err.message : String(err),
    });
    return { outcome: "failed" };
